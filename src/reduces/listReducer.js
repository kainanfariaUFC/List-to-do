import {CONSTANTS} from '../actions'

let listID = 2;
let cardID = 8;

const initialState = [
    {
        title: "Fazendo",
        id: `list=${0}`,
        cards: [
            {
                id: `card=${0}`,
                text: "Backend DB"
            },
            {
                id: `card=${1}`,
                text: "Edit CARD's content"
            },
            {
                id: `card=${2}`,
                text: "Edit LIST's title"
            }
        ]

    },
    {
        title: "Concluído",
        id: `list=${1}`,
        cards: [
            {
                id: `card=${3}`,
                text: "Adicionar listas"
            },
            {
                id: `card=${4}`,
                text: "Adicionar Cards"
            },
            {
                id: `card=${5}`,
                text: "Drag and drop"
            },
            {
                id: `card=${6}`,
                text: "Listas e cards expansíveis dependendo do conteúdo"
            },
            {
                id: `card=${7}`,
                text: "Funcionando dentro de uma aplicação Flask"
            },
            
        ]

    }
]

const listReducer = (state = initialState, action) => {

    switch(action.type){
        case CONSTANTS.ADD_LIST:
            const newList ={
                title: action.payload.title,
                id: `list=${listID}`,
                cards: [],
            }
            listID += 1;
            return [...state, newList]
        
        case CONSTANTS.ADD_CARD:
            const newCard={
                text: action.payload.text,
                id: `card=${cardID}`,
            };
            cardID += 1;
            const newStates = state.map(list =>{
                if(list.id === action.payload.listID){
                    return{
                        ...list,
                        cards: [...list.cards, newCard]
                    }
                }else{
                    return list
                }
            });

            return newStates;

        case CONSTANTS.DRAG_HAPPEN:
            const {
                droppableIdStart, 
                droppableIdEnd,
                droppableIndexStart,
                droppableIndexEnd,
            } = action.payload;

            const newState = [...state];

    
            if(droppableIdStart === droppableIdEnd){
                const list = state.find(list => droppableIdStart === list.id.toString());
                const card = list.cards.splice(droppableIndexStart, 1);
                list.cards.splice(droppableIndexEnd, 0, ...card);
            }

            if(droppableIdStart !== droppableIdEnd){
                const listStart = state.find(list => droppableIdStart === list.id.toString());
                const card = listStart.cards.splice(droppableIndexStart, 1);
                const listEnd = state.find(list => droppableIdEnd === list.id.toString());
                listEnd.cards.splice(droppableIndexEnd, 0, ...card);

            }

            return newState;


        default:
            return state;
    }
}

export default listReducer;